...
final class CommunityIntelligenceAutoPublisher
{
    ...
    public static function evaluate(PDO $pdo,int $productId,array $candidate,array $prior=[]): array
    {
        ...
        $gates=[
            ...
            'source_diversity'=>['pass'=>$types>=$minD&&$metrics['domain_count']>=$minD,'value'=>['types'=>$types,'domains'=>$metrics['domain_count']],'required'=>['types'=>$minD,'domains'=>$minD]],
            'confidence'=>['pass'=>$confidence>=(float)$policy['min_confidence'],'value'=>$confidence,'required'=>(float)$policy['min_confidence']],
            'domain_concentration'=>['pass'=>$metrics['max_domain_share']<=(float)$policy['max_single_domain_share'],'value'=>$metrics['max_domain_share'],'required'=>(float)$policy['max_single_domain_share']],
            'recent_sources'=>['pass'=>$metrics['recent_count']>=(int)$policy['min_recent_sources'],'value'=>$metrics['recent_count'],'required'=>(int)$policy['min_recent_sources']],
            'source_policy_clear'=>['pass'=>$metrics['unresolved_policy_count']===0,'value'=>$metrics['unresolved_policy_count'],'required'=>0],
            'suspicious_mix'=>['pass'=>$metrics['suspicious_ratio']<=(float)$policy['max_suspicious_ratio'],'value'=>$metrics['suspicious_ratio'],'required'=>(float)$policy['max_suspicious_ratio']],
    ...
    {
        $st=$pdo->prepare("SELECT * FROM product_public_review_intelligence WHERE product_id=? LIMIT 1");$st->execute([$productId]);$row=$st->fetch(PDO::FETCH_ASSOC);if(!$row)throw new RuntimeException('Community Intelligence record not found.');$candidate=$row;$candidate['strengths']=json_decode($row['strengths_json']??'[]',true)?:[];$candidate['concerns']=json_decode($row['concerns_json']??'[]',true)?:[];$candidate['themes']=json_decode($row['themes_json']??'{}',true)?:[];$decision=self::evaluate($pdo,$productId,$candidate,$row);self::applyDecision($pdo,$productId,$row,$decision,isset($row['analysis_run_id'])?(int)$row['analysis_run_id']:null);return $decision;
    }
