...
final class EvidenceRefreshScheduler{
  ...
  public static function run(PDO $pdo,int $intervalHours=self::DEFAULT_INTERVAL_HOURS,int $limit=self::DEFAULT_LIMIT):array{
    ...
    try{
      $sql="SELECT es.id FROM evidence_sources es JOIN products p ON p.id=es.product_id WHERE p.status='active' AND TRIM(COALESCE(es.source_url,''))<>'' AND es.checked_at<=DATE_SUB(NOW(),INTERVAL {$intervalHours} HOUR) ORDER BY es.checked_at ASC,es.id ASC LIMIT {$limit}";
      ...
      foreach($ids as $sourceId){
        ...
        if(isset($summary[$state]))$summary[$state]++;else $summary['errors']++;
