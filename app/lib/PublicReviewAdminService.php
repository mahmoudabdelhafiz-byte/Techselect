...
require_once __DIR__.'/CommunityIntelligenceAutoPublisher.php';
...
final class PublicReviewAdminService
{
    ...
    public static function analyzeProduct(PDO $pdo,int $productId,array $snapshots):array
    {
        ...
        $priorSt=$pdo->prepare("SELECT * FROM product_public_review_intelligence WHERE product_id=? LIMIT 1");$priorSt->execute([$productId]);$prior=$priorSt->fetch(PDO::FETCH_ASSOC)?:[];
        ...
        try{
            ...
            $rows=self::latestSignals($pdo,$productId);$intelligence=PublicReviewIntelligence::calculate($rows);[$strengths,$concerns,$themes,$mix,$start,$end]=self::summaries($rows);
            $candidate=$intelligence+['strengths'=>$strengths,'concerns'=>$concerns,'themes'=>$themes];
            $auto=CommunityIntelligenceAutoPublisher::evaluate($pdo,$productId,$candidate,$prior);
            $snapshot=['analysis_run_id'=>$runId,'score_5'=>$intelligence['score_5'],'positive_sentiment_pct'=>$intelligence['positive_sentiment_pct'],'confidence_score'=>$intelligence['confidence_score'],'confidence_label'=>$intelligence['confidence_label'],'sources_analyzed'=>$intelligence['sources_analyzed'],'source_type_count'=>$intelligence['source_type_count'],'insufficient_data'=>$intelligence['insufficient_data']?1:0,'strengths'=>$strengths,'concerns'=>$concerns,'themes'=>$themes,'source_mix'=>$mix,'date_range_start'=>$start,'date_range_end'=>$end,'methodology_version'=>$intelligence['methodology_version'],'last_analyzed_at'=>gmdate('Y-m-d H:i:s')];
            $priorPublished=(($prior['review_status']??'')==='published'&&!empty($prior['published_at']));
            if($priorPublished&&!$auto['eligible']){
                $pdo->prepare("UPDATE product_public_review_intelligence SET auto_publish_candidate_json=? WHERE product_id=?")->execute([json_encode($snapshot,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),$productId]);
                CommunityIntelligenceAutoPublisher::applyDecision($pdo,$productId,$prior,$auto,$runId);
            }else{
                self::saveIntelligence($pdo,$productId,$snapshot);
                if(!$auto['eligible'])$pdo->prepare("UPDATE product_public_review_intelligence SET auto_publish_candidate_json=? WHERE product_id=?")->execute([json_encode($snapshot,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),$productId]);
                CommunityIntelligenceAutoPublisher::applyDecision($pdo,$productId,$prior,$auto,$runId);
                if($auto['eligible'])$pdo->prepare("UPDATE product_public_review_intelligence SET auto_publish_candidate_json=NULL WHERE product_id=?")->execute([$productId]);
            }
            return ['analysis_run_id'=>$runId,'eligible'=>$eligible,'excluded'=>$excluded,'errors'=>$errors,'intelligence'=>$intelligence,'strengths'=>$strengths,'concerns'=>$concerns,'themes'=>$themes,'source_mix'=>$mix,'date_range'=>[$start,$end],'auto_publication'=>$auto];
    ...
    public static function publishHeldCandidate(PDO $pdo,int $productId): bool
    {
        $st=$pdo->prepare("SELECT auto_publish_candidate_json FROM product_public_review_intelligence WHERE product_id=? LIMIT 1");$st->execute([$productId]);$raw=$st->fetchColumn();if(!$raw)return false;$snapshot=json_decode((string)$raw,true);if(!is_array($snapshot))return false;self::saveIntelligence($pdo,$productId,$snapshot);$pdo->prepare("UPDATE product_public_review_intelligence SET auto_publish_candidate_json=NULL WHERE product_id=?")->execute([$productId]);return true;
    }
    private static function saveIntelligence(PDO $pdo,int $productId,array $s):void
    {
        $sql="INSERT INTO product_public_review_intelligence(product_id,analysis_run_id,score_5,positive_sentiment_pct,confidence_score,confidence_label,sources_analyzed,source_type_count,insufficient_data,strengths_json,concerns_json,themes_json,source_mix_json,date_range_start,date_range_end,methodology_version,last_analyzed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE analysis_run_id=VALUES(analysis_run_id),score_5=VALUES(score_5),positive_sentiment_pct=VALUES(positive_sentiment_pct),confidence_score=VALUES(confidence_score),confidence_label=VALUES(confidence_label),sources_analyzed=VALUES(sources_analyzed),source_type_count=VALUES(source_type_count),insufficient_data=VALUES(insufficient_data),strengths_json=VALUES(strengths_json),concerns_json=VALUES(concerns_json),themes_json=VALUES(themes_json),source_mix_json=VALUES(source_mix_json),date_range_start=VALUES(date_range_start),date_range_end=VALUES(date_range_end),methodology_version=VALUES(methodology_version),last_analyzed_at=VALUES(last_analyzed_at)";
        $pdo->prepare($sql)->execute([$productId,$s['analysis_run_id'],$s['score_5'],$s['positive_sentiment_pct'],$s['confidence_score'],$s['confidence_label'],$s['sources_analyzed'],$s['source_type_count'],$s['insufficient_data'],json_encode($s['strengths']??[]),json_encode($s['concerns']??[]),json_encode($s['themes']??[]),json_encode($s['source_mix']??[]),$s['date_range_start']??null,$s['date_range_end']??null,$s['methodology_version'],$s['last_analyzed_at']??gmdate('Y-m-d H:i:s')]);
    }
    private static function latestSignals(PDO $pdo,int $productId):array
    {
        $sql="SELECT s.id source_id,s.source_url,s.source_type,s.source_published_at,s.access_policy,s.status,sg.sentiment_score,sg.sentiment_label,sg.public_rating,sg.public_rating_scale,sg.topic_json,sg.themes_json,sg.reviewer_context_json,sg.source_confidence,sg.source_quality,sg.independence_score,sg.specificity_score,sg.duplicate_suspected,sg.spam_suspected,sg.affiliate_suspected,sg.vendor_promotion_suspected,sg.bot_suspected,sg.low_signal_suspected,sg.exclusion_reason FROM public_review_sources s JOIN public_review_signals sg ON sg.id=(SELECT MAX(sg2.id) FROM public_review_signals sg2 WHERE sg2.source_id=s.id) WHERE s.product_id=? AND s.status='active' AND s.access_policy='permitted'";$st=$pdo->prepare($sql);$st->execute([$productId]);$rows=$st->fetchAll();foreach($rows as &$r){$r['topics']=json_decode($r['topic_json']??'[]',true)?:[];$r['themes']=json_decode($r['themes_json']??'{}',true)?:[];}unset($r);return array_values(array_filter($rows,fn($r)=>PublicReviewIngestion::isEligibleSignal($r)));
    }
