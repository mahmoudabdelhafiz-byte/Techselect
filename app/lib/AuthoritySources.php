...
final class AuthoritySources {
  ...
  private const READINESS=['researching','ready','eligibility_check','blocked','submitted'];

  public static function list(PDO $pdo):array{
    $sql="SELECT a.*,u.email owner_email FROM authority_sources a LEFT JOIN users u ON u.id=a.owner_user_id ORDER BY CASE a.readiness_status WHEN 'ready' THEN 0 WHEN 'eligibility_check' THEN 1 WHEN 'researching' THEN 2 WHEN 'blocked' THEN 3 ELSE 4 END,CASE a.backlink_status WHEN 'active' THEN 0 ELSE 1 END,a.next_action_at IS NULL,a.next_action_at,a.updated_at DESC";
    return $pdo->query($sql)->fetchAll();
  }

  public static function summary(PDO $pdo):array{
    $r=$pdo->query("SELECT COUNT(*) total,SUM(outreach_status='prospect') prospects,SUM(outreach_status IN('contacted','follow_up')) contacted,SUM(outreach_status='published') published,SUM(backlink_status='active') active_backlinks,SUM(backlink_status='lost') lost_backlinks,SUM(readiness_status='ready') ready,SUM(readiness_status='eligibility_check') eligibility_check FROM authority_sources")->fetch()?:[];
    ...
    return ['total'=>(int)($r['total']??0),'prospects'=>(int)($r['prospects']??0),'contacted'=>(int)($r['contacted']??0),'published'=>(int)($r['published']??0),'active_backlinks'=>(int)($r['active_backlinks']??0),'lost_backlinks'=>(int)($r['lost_backlinks']??0),'ready'=>(int)($r['ready']??0),'eligibility_check'=>(int)($r['eligibility_check']??0),'by_type'=>$types];
  }
  ...
  public static function save(PDO $pdo,array $b,int $actor,?int $id=null):array{
    ...
    $readiness=(string)($b['readiness_status']??'researching');if(!in_array($readiness,self::READINESS,true))throw new InvalidArgumentException('invalid_readiness_status');
    ...
    $sourceUrl=self::urlOrNull($b['source_url']??null);$submissionUrl=self::urlOrMailtoNull($b['submission_url']??null);$routeSourceUrl=self::urlOrNull($b['route_source_url']??null);$publishedUrl=self::urlOrNull($b['published_url']??null);
    ...
    if($readiness==='submitted'&&!in_array($outreach,['contacted','follow_up','accepted','published'],true))throw new InvalidArgumentException('submitted_readiness_requires_outreach_state');
    $fields=['source_name'=>$sourceName,'domain'=>$domain,'source_url'=>$sourceUrl,'submission_url'=>$submissionUrl,'route_source_url'=>$routeSourceUrl,'source_type'=>$sourceType,'geography_code'=>self::nullable($b['geography_code']??null,10),'relevance_score'=>$relevance,'authority_tier'=>$tier,'owner_user_id'=>self::nullableInt($b['owner_user_id']??null),'outreach_status'=>$outreach,'next_action'=>self::nullable($b['next_action']??null,255),'next_action_at'=>self::nullableDate($b['next_action_at']??null),'techselect_asset_path'=>self::assetPath($b['techselect_asset_path']??null),'intended_context'=>self::nullable($b['intended_context']??null,500),'prerequisites'=>self::nullable($b['prerequisites']??null,4000),'required_assets'=>self::nullable($b['required_assets']??null,4000),'readiness_status'=>$readiness,'route_verified_at'=>self::nullableDate($b['route_verified_at']??null),'mention_requires_approval'=>$requires,'mention_approved'=>$approved,'approval_notes'=>self::nullable($b['approval_notes']??null,500),'published_url'=>$publishedUrl,'link_type'=>$linkType!==''?$linkType:null,'rel_attribute'=>self::nullable($b['rel_attribute']??null,80),'backlink_status'=>$backlink,'first_verified_at'=>self::nullableDate($b['first_verified_at']??null),'last_checked_at'=>self::nullableDate($b['last_checked_at']??null),'notes'=>self::nullable($b['notes']??null,4000)];
  ...
  private static function urlOrMailtoNull($v):?string{$v=trim((string)$v);if($v==='')return null;if(str_starts_with(strtolower($v),'mailto:'))return filter_var(substr($v,7),FILTER_VALIDATE_EMAIL)?$v:throw new InvalidArgumentException('invalid_submission_url');return self::urlOrNull($v);}
