...
final class CommunitySourceCollectors
{
    ...
    {
        self::validatePublicUrl($url);if(!function_exists('curl_init'))throw new RuntimeException('curl_required');$ch=curl_init($url);curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_FOLLOWLOCATION=>false,CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>20,CURLOPT_USERAGENT=>'TechSelectAI-CommunityCollector/1.0',CURLOPT_HTTPHEADER=>['Accept: application/json, application/rss+xml, application/atom+xml, application/xml, text/xml;q=0.9'],CURLOPT_PROTOCOLS=>CURLPROTO_HTTP|CURLPROTO_HTTPS]);$body=curl_exec($ch);if($body===false){$e=curl_error($ch);curl_close($ch);throw new RuntimeException('http_fetch_failed: '.$e);}if(strlen($body)>$maxBytes){curl_close($ch);throw new RuntimeException('response_too_large');}$status=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);$effective=(string)curl_getinfo($ch,CURLINFO_EFFECTIVE_URL);curl_close($ch);self::validatePublicUrl($effective);if($status>=300&&$status<400)throw new RuntimeException('redirect_not_followed_for_ssrf_safety');if($status<200||$status>=300)throw new RuntimeException('upstream_http_'.$status);return ['body'=>$body,'status'=>$status,'url'=>$effective];
    }
