...
final class BuyerIntentAnalytics {
  public static function recordPublicEvent(PDO $pdo,string $eventType,array $data=[]): void {
    ...
    try {
      ...
      $st->execute([
        ...
        isset($data['related_product_id'])?(int)$data['related_product_id']:null,
        isset($data['route_path'])?substr((string)$data['route_path'],0,255):null
      ]);
    } catch(Throwable $e) {
      // Analytics must never break public pages before/after migration rollout.
    }
  }
  ...
  public static function dashboard(PDO $pdo,int $days=30): array {
    ...
    $views=[];$comparisons=[];
    try {
      $views=$rows("SELECT p.name,p.slug,COUNT(*) view_count FROM buyer_intent_events e JOIN products p ON p.id=e.product_id WHERE e.occurred_at>=? AND e.event_type='product_view' GROUP BY p.id,p.name,p.slug ORDER BY view_count DESC LIMIT 20",[$since]);
      $comparisons=$rows("SELECT p1.name product_a,p1.slug slug_a,p2.name product_b,p2.slug slug_b,COUNT(*) comparison_count FROM buyer_intent_events e JOIN products p1 ON p1.id=e.product_id JOIN products p2 ON p2.id=e.related_product_id WHERE e.occurred_at>=? AND e.event_type='comparison_view' GROUP BY p1.id,p1.name,p1.slug,p2.id,p2.name,p2.slug ORDER BY comparison_count DESC LIMIT 20",[$since]);
    } catch(Throwable $e) {}

    $unmapped=[];
    try{$unmapped=$rows("SELECT normalized_topic label,demand_count,last_seen_at,status,priority FROM taxonomy_expansion_queue ORDER BY demand_count DESC,last_seen_at DESC LIMIT 20");}catch(Throwable $e){}

    return compact('days','overview','categories','requirements','products','countries','industries','companySizes','views','comparisons','unmapped');
  }
}
