...
final class VendorPortfolio {
    ...
    public static function vendorProfile(PDO $pdo,string $slug): ?array {
        ...
        $vendor['partner_products']=self::relationshipsForVendor($pdo,(int)$vendor['id'],['authorized_reseller','distributor','implementation_partner','referral_partner','local_agent']);
        return $vendor;
    }
    ...
    public static function partnersForProduct(PDO $pdo,int $productId,?string $territoryCode=null): array {
        $sql="SELECT r.id,r.relationship_type,r.verification_status,r.valid_from,r.valid_until,r.evidence_source_url,r.evidence_note,v.id vendor_id,v.name vendor_name,v.slug vendor_slug,v.website_url,v.verification_status vendor_verification FROM vendor_product_relationships r JOIN vendors v ON v.id=r.vendor_id WHERE r.product_id=? AND r.status='approved' AND r.relationship_type NOT IN ('software_owner','developer')";
