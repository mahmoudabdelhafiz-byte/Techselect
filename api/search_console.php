...
if($path==='/api/search-console/import'&&$method==='POST'){
  $b=sc_body();$rows=$b['rows']??[];$indexRows=$b['index_rows']??[];
  if(!is_array($rows)||count($rows)>50000||!is_array($indexRows)||count($indexRows)>10000)sc_out(['error'=>'invalid_rows'],422);
  try{
    $r=SearchConsoleAnalytics::importRows($pdo,$rows,$b,(int)$user['id']);$indexCount=0;
    if($indexRows)$indexCount=IndexationHealth::importIndexStates($pdo,$indexRows,'google_search_console');
    $r['index_rows_imported']=$indexCount;
    Security::audit($pdo,(int)$user['id'],'SEARCH_CONSOLE_IMPORT','search_console_import',(string)$r['import_id'],null,['source_label'=>$b['source_label']??null,'date_from'=>$b['date_from']??null,'date_to'=>$b['date_to']??null,'rows_imported'=>$r['rows_imported'],'index_rows_imported'=>$indexCount]);
    sc_out($r,201);
  }catch(Throwable $e){sc_out(['error'=>$e instanceof InvalidArgumentException?$e->getMessage():'import_failed'],422);}
}
