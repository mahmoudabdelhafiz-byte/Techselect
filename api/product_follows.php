...
if($method==='GET'){if(!$user)pf_out(['authenticated'=>false,'followed'=>false,'product'=>['slug'=>$product['slug'],'name'=>$product['name']]]);Security::csrf();$state=ProductFollows::state($pdo,(int)$user['id'],(int)$product['id']);pf_out(['authenticated'=>true,'followed'=>$state['followed'],'product'=>['slug'=>$product['slug'],'name'=>$product['name']]]);}
...
if($method==='POST'){$state=ProductFollows::follow($pdo,(int)$user['id'],(int)$product['id']);Security::audit($pdo,(int)$user['id'],'PRODUCT_FOLLOW','product',(string)$product['id'],null,['slug'=>$product['slug']]);pf_out(['followed'=>$state['followed']],201);}
if($method==='DELETE'){$state=ProductFollows::unfollow($pdo,(int)$user['id'],(int)$product['id']);Security::audit($pdo,(int)$user['id'],'PRODUCT_UNFOLLOW','product',(string)$product['id'],null,['slug'=>$product['slug']]);pf_out(['followed'=>$state['followed']]);}
