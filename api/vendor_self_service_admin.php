...
if($method!=='GET'){Security::sameOrigin($config);Security::requireCsrf();Security::rateLimit($pdo,'vendor-self-service-admin-write',30,60);}
...
if(preg_match('#^/api/vendor-self-service-admin/claims/(\d+)/(verify|reject)$#',$path,$m)&&$method==='POST'){
 $id=(int)$m[1];$action=$m[2];$st=$pdo->prepare('SELECT * FROM vendor_profile_claims WHERE id=?');$st->execute([$id]);$c=$st->fetch(PDO::FETCH_ASSOC);if(!$c)vsa_out(['error'=>'not_found'],404);$b=vsa_body();
...
if(preg_match('#^/api/vendor-self-service-admin/updates/(\d+)/(approve|reject)$#',$path,$m)&&$method==='POST'){
 $id=(int)$m[1];$action=$m[2];$st=$pdo->prepare('SELECT * FROM vendor_update_requests WHERE id=?');$st->execute([$id]);$r=$st->fetch(PDO::FETCH_ASSOC);if(!$r)vsa_out(['error'=>'not_found'],404);$b=vsa_body();$note=trim((string)($b['note']??''));
