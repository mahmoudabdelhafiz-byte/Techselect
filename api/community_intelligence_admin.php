...
if($path==='/api/community-intelligence/auto-publish-settings'&&$method==='GET'){
  ci_out(['settings'=>CommunityIntelligenceAutoPublisher::settings($pdo),'csrf_token'=>Security::csrf()]);
}
if($path==='/api/community-intelligence/auto-publish-settings'&&$method==='POST'){
  if(!in_array($u['role']??'', ['admin','super_admin'],true))ci_out(['error'=>'admin_required'],403);$before=CommunityIntelligenceAutoPublisher::settings($pdo);
  try{$settings=CommunityIntelligenceAutoPublisher::updateSettings($pdo,ci_body(),(int)$u['id']);}catch(Throwable $e){ci_out(['error'=>$e->getMessage()],422);}
  Security::audit($pdo,(int)$u['id'],'COMMUNITY_INTELLIGENCE_AUTO_POLICY_UPDATE','community_intelligence','1',$before,$settings);ci_out(['settings'=>$settings]);
}
if($path==='/api/community-intelligence'&&$method==='GET'){
  ...
  $rows=$pdo->query($sql)->fetchAll();foreach($rows as &$r){foreach(['strengths_json'=>'strengths','concerns_json'=>'concerns','themes_json'=>'themes','source_mix_json'=>'source_mix','auto_publish_decision_json'=>'auto_publish_decision'] as $col=>$out){$r[$out]=json_decode($r[$col]??'[]',true)?:[];unset($r[$col]);}}unset($r);ci_out(['items'=>$rows,'auto_publish_settings'=>CommunityIntelligenceAutoPublisher::settings($pdo),'csrf_token'=>Security::csrf()]);
}
if(preg_match('#^/api/community-intelligence/(\d+)$#',$path,$m)&&$method==='GET'){
  ...
  foreach(['strengths_json'=>'strengths','concerns_json'=>'concerns','themes_json'=>'themes','source_mix_json'=>'source_mix','auto_publish_decision_json'=>'auto_publish_decision','auto_publish_candidate_json'=>'held_candidate'] as $col=>$out){$item[$out]=json_decode($item[$col]??'[]',true)?:[];unset($item[$col]);}
...
if(preg_match('#^/api/community-intelligence/(\d+)/auto-hold$#',$path,$m)&&$method==='POST'){
  $pid=(int)$m[1];$b=ci_body();$reason=trim((string)($b['reason']??''));try{CommunityIntelligenceAutoPublisher::setManualHold($pdo,$pid,$reason);}catch(Throwable $e){ci_out(['error'=>$e->getMessage()],422);}
  Security::audit($pdo,(int)$u['id'],'COMMUNITY_INTELLIGENCE_AUTO_HOLD','product',(string)$pid,null,['reason'=>$reason]);ci_out(['updated'=>true,'manual_hold_reason'=>$reason?:null]);
}
if(preg_match('#^/api/community-intelligence/(\d+)/auto-evaluate$#',$path,$m)&&$method==='POST'){
  $pid=(int)$m[1];try{$decision=CommunityIntelligenceAutoPublisher::evaluateExisting($pdo,$pid);}catch(Throwable $e){ci_out(['error'=>$e->getMessage()],422);}
  Security::audit($pdo,(int)$u['id'],'COMMUNITY_INTELLIGENCE_AUTO_EVALUATE','product',(string)$pid,null,['decision'=>$decision['decision'],'failed_gates'=>$decision['failed_gates']]);ci_out(['decision'=>$decision]);
}
if(preg_match('#^/api/community-intelligence/(\d+)/review$#',$path,$m)&&$method==='POST'){
  ...
  $candidate=!empty($old['auto_publish_candidate_json'])?json_decode((string)$old['auto_publish_candidate_json'],true):null;$threshold=$candidate?:$old;
  if($action==='approve'){
    if(!empty($threshold['insufficient_data']) || (int)($threshold['sources_analyzed']??0)<3 || (int)($threshold['source_type_count']??0)<2 || (float)($threshold['confidence_score']??0)<0.65)ci_out(['error'=>'publication_threshold_not_met'],409);
  ...
  try{
    $pdo->beginTransaction();$snapshot=json_encode($old,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
    ...
    if($action==='approve'&&$candidate)PublicReviewAdminService::publishHeldCandidate($pdo,$pid);
    $clearCandidate=$action==='approve'||$action==='reject'||$action==='request_more_evidence';
    $sql="UPDATE product_public_review_intelligence SET review_status=?,review_notes=?,reviewed_at=NOW(),reviewed_by_user_id=?,published_at={$published},publication_mode=?,auto_publish_hold_reason=NULL".($clearCandidate?',auto_publish_candidate_json=NULL':'')." WHERE product_id=?";
    $pdo->prepare($sql)->execute([$status,$notes!==''?$notes:null,(int)$u['id'],$action==='approve'?'manual':null,$pid]);$pdo->commit();
  }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();ci_out(['error'=>$e->getMessage()],422);}
  Security::audit($pdo,(int)$u['id'],'COMMUNITY_INTELLIGENCE_REVIEW','product',(string)$pid,$old,['action'=>$action,'status'=>$status,'notes'=>$notes,'held_candidate_applied'=>(bool)($action==='approve'&&$candidate)]);ci_out(['updated'=>true,'status'=>$status]);
}
