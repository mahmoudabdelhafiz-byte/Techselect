<?php
/** Adds visible factual summaries, source transparency/freshness signals, and matching JSON-LD to software/comparison pages. */
ob_start();
...
try{
  ...
  require_once __DIR__.'/app/lib/PublicTransparency.php';
  $cfg=require __DIR__.'/app/config.php';
  $pdo=Db::pdo();
  $fact=AiFactualSummary::build($pdo,$path,(string)($cfg['site_url']??'https://techselectai.com'));
  ...
  $transparency=PublicTransparency::build($pdo,$path);
  if($transparency && stripos($html,'<html')!==false){
    $html=str_ireplace('</head>',PublicTransparency::css().'</head>',$html);
    $section=PublicTransparency::render($transparency);
    if(stripos($html,'<section class="cta">')!==false)$html=preg_replace('#<section class="cta">#',$section.'<section class="cta">',$html,1)??$html;
    else $html=str_ireplace('</main>',$section.'</main>',$html);
  }
}catch(Throwable $e){}
echo $html;
