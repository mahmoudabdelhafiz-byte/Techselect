...
function signedIn(nav,user,csrf){
 ...
 nav.innerHTML=`<span class="name" title="${esc(label)}">${esc(label)}</span><a href="/my-consultations">My Consultations</a><a href="/my-followed-products">Following</a><a href="/my-business-cases">My Business Cases</a><a href="/my-reviews">My Reviews</a>${evidence}<a href="/account-settings">Settings</a><button type="button" data-logout>Log out</button>`;
