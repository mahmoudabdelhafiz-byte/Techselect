...
elseif(in_array($path,['/my-followed-products','/my-followed-products/','/my_followed_products.php'],true)) $target='my_followed_products.php';
...
if($isPublicKnowledge){
  ...
  if(preg_match('#^/software/([a-z0-9-]+)/?$#',$path,$bm)){
    $slug=htmlspecialchars($bm[1],ENT_QUOTES,'UTF-8');
    $businessCase='<section class="ts-bc-public"><h2>Build a Business Case</h2><p>Turn this product evaluation into a management-ready justification. Known consultation context can be reused, and verified product facts stay separate from assumptions and estimates.</p><a href="/build-business-case?product='.$slug.'">Build a Business Case</a></section>';
    $html=preg_replace('#<section class="cta">#',$businessCase.'<section class="cta">',$html,1)??$html;
  }
}
