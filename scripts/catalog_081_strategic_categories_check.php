<?php

declare(strict_types=1);

$root = dirname(__DIR__);
$migration = $root . '/db/mysql/081_add_strategic_categories_batch1.sql';
if (!is_file($migration)) {
    fwrite(STDERR, "Missing migration: {$migration}\n");
    exit(1);
}
$sql = file_get_contents($migration) ?: '';
$failures = [];
$require = static function (bool $ok, string $message) use (&$failures): void {
    if (!$ok) $failures[] = $message;
};

$categories = [
    'enterprise-architecture-apm',
    'integration-api-management',
    'rpa-intelligent-automation',
];
foreach ($categories as $slug) {
    $require(substr_count($sql, "'{$slug}'") >= 2, "Category {$slug} is not consistently defined/reused.");
}

$modules = [
    'ea-modeling-repository','ea-portfolio-governance',
    'integration-orchestration','api-management-governance',
    'rpa-execution','rpa-governance-intelligence',
];
foreach ($modules as $slug) {
    $require(strpos($sql, "'{$slug}'") !== false, "Missing module {$slug}.");
}

$capabilities = [
    'ea-application-inventory','ea-business-capability-mapping','ea-dependency-modeling','ea-roadmaps-target-state',
    'ea-application-lifecycle','ea-portfolio-rationalization','ea-risk-cost-assessment','ea-collaboration-governance',
    'integration-connectors','integration-workflow-orchestration','integration-data-transformation','integration-hybrid-runtime',
    'api-design-publish','api-gateway-security','api-lifecycle-governance','api-monitoring-analytics',
    'rpa-desktop-web-automation','rpa-attended','rpa-unattended','rpa-orchestration-scheduling',
    'rpa-process-workflow','rpa-ai-document','rpa-governance-audit','rpa-human-in-loop',
];
foreach ($capabilities as $slug) {
    $require(strpos($sql, "'{$slug}'") !== false, "Missing capability {$slug}.");
}

$products = [
    'sap-leanix' => 'enterprise-architecture-apm',
    'ardoq' => 'enterprise-architecture-apm',
    'bizzdesign-horizzon' => 'enterprise-architecture-apm',
    'orbusinfinity' => 'enterprise-architecture-apm',
    'mulesoft-anypoint-platform' => 'integration-api-management',
    'boomi-enterprise-platform' => 'integration-api-management',
    'workato' => 'integration-api-management',
    'snaplogic' => 'integration-api-management',
    'uipath-platform' => 'rpa-intelligent-automation',
    'automation-anywhere' => 'rpa-intelligent-automation',
    'microsoft-power-automate' => 'rpa-intelligent-automation',
];
foreach ($products as $slug => $category) {
    $require(substr_count($sql, "'{$slug}'") >= 2, "Product {$slug} is not consistently referenced.");
    $require(strpos($sql, "'{$category}'") !== false, "Category {$category} is not referenced for {$slug}.");
}

$require(strpos($sql, 'ON DUPLICATE KEY UPDATE') !== false, '081 must be idempotent.');
$require(strpos($sql, 'last_reviewed_at') !== false, 'Products must update last_reviewed_at.');
$require(strpos($sql, "'not_yet_verified'") !== false, 'Unreviewed facts must remain not_yet_verified.');
$require(strpos($sql, 'product_capability_evidence') !== false, 'Known facts must link to evidence.');
$require(strpos($sql, "1,'verified','high'") !== false, 'Evidence must be vendor-owned, verified and high confidence.');
$require(stripos($sql, 'g2.com') === false, 'G2 data must not be ingested.');
$require(stripos($sql, 'capterra') === false, 'Capterra data must not be ingested.');
$require(stripos($sql, 'preferred_vendor') === false, 'Catalog migration must not add preferred-vendor logic.');
$require(stripos($sql, 'fit_score') === false, 'Catalog migration must not alter Fit Score.');

preg_match_all("/\\('(?:[^']|'')+','(?:[^']|'')+','(?:supported|partially_supported)',[0-9.]+,(?:NULL|'(?:[^']|'')*'),'([^']+)'\\)/", $sql, $matches);
foreach ($matches[1] ?? [] as $url) {
    $require(substr_count($sql, "'{$url}'") >= 2, "Known fact source is not represented in evidence: {$url}");
}

$officialHosts = [
    'leanix.net','help.sap.com','sap.com','ardoq.com','bizzdesign.com','orbussoftware.com',
    'mulesoft.com','anypoint.mulesoft.com','boomi.com','developer.boomi.com','workato.com','docs.workato.com',
    'snaplogic.com','docs.snaplogic.com','uipath.com','docs.uipath.com','automationanywhere.com','docs.automationanywhere.com',
    'microsoft.com','learn.microsoft.com'
];
preg_match_all("/'https:\\/\\/([^\\/']+)[^']*'/", $sql, $urls);
foreach ($urls[1] ?? [] as $host) {
    $host = strtolower($host);
    $ok = false;
    foreach ($officialHosts as $allowed) {
        if ($host === $allowed || str_ends_with($host, '.' . $allowed)) { $ok = true; break; }
    }
    $require($ok, "Unexpected non-first-party host in migration: {$host}");
}

$migrationFiles = glob($root . '/db/mysql/*.sql') ?: [];
foreach (array_keys($products) as $slug) {
    $hits = 0;
    foreach ($migrationFiles as $file) {
        if ($file === $migration) continue;
        $text = file_get_contents($file) ?: '';
        if (strpos($text, "'{$slug}'") !== false) $hits++;
    }
    $require($hits === 0, "Potential duplicate product shell already exists for {$slug}.");
}

$require(strpos($sql, "SELECT p.id,i.id,'not_yet_verified',0") !== false, 'Integration support must remain unknown when not reviewed.');
$require(substr_count($sql, "INSERT INTO categories") === 1, '081 should create/update categories in one controlled block.');
$require(substr_count($sql, "INSERT INTO modules") === 1, '081 should create/update modules in one controlled block.');
$require(substr_count($sql, "INSERT INTO capabilities") === 1, '081 should create/update capabilities in one controlled block.');

if ($failures !== []) {
    foreach ($failures as $failure) fwrite(STDERR, "FAIL: {$failure}\n");
    exit(1);
}

echo "Catalog 081 strategic-category contract passed.\n";
