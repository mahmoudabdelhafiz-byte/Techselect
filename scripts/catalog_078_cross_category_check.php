...
$migration=$root.'/db/mysql/078_expand_major_categories_batch5.sql';
...
$require(stripos($sql,'INSERT INTO categories')===false,'078 must not create categories.');
$require(stripos($sql,'INSERT INTO modules')===false,'078 must not create modules.');
$require(stripos($sql,'INSERT INTO capabilities')===false,'078 must not create capabilities.');

$products=[
 'sugar-sell'=>'crm',
 'topdesk'=>'itsm',
 'paylocity'=>'hr-hcm',
 'acumatica-cloud-erp'=>'erp',
 'airtable'=>'project-management',
 'sap-analytics-cloud'=>'business-intelligence-analytics',
 'trellix-endpoint-security'=>'endpoint-security',
 'nakivo-backup-replication'=>'backup-disaster-recovery',
];
...
$require(strpos($sql,'ON DUPLICATE KEY UPDATE')!==false,'078 must be idempotent.');
...
foreach($matches[1]??[] as $url){$require(substr_count($sql,"'{$url}'")>=2,"Known fact source is not represented in evidence: {$url}");}

$officialHosts=['sugarcrm.com','support.sugarcrm.com','topdesk.com','paylocity.com','acumatica.com','airtable.com','support.airtable.com','sap.com','help.sap.com','trellix.com','nakivo.com','helpcenter.nakivo.com','download.nakivo.com'];
...
$require(strpos($sql,"'public-saas'")!==false,'078 must include conservative SaaS deployment facts.');
$require(strpos($sql,"'on-premise'")!==false,'078 must include conservative on-premise deployment facts where verified.');
...
echo "Catalog 078 cross-category contract passed.\n";
