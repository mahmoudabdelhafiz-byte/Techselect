...
$backfill=file_get_contents($root.'/db/mysql/137_mobile_access_post087_backfill.sql')?:'';
...
foreach(['Mobile Access','mobile-android-app','mobile-ios-app','mobile-web-access','product_mobile_access','pma.support_status','pma.confidence_score','not_yet_verified'] as $term){
  if(!str_contains($backfill,$term))$errors[]="137 backfill missing {$term}";
}
if(str_contains($backfill,'UPDATE product_mobile_access')||str_contains($backfill,'INSERT INTO product_mobile_access'))$errors[]='137 backfill must project mobile evidence without mutating canonical product_mobile_access facts';
if(str_contains($backfill,"support_status='supported'"))$errors[]='137 backfill must not infer mobile support';
